...
#define REWARD_WIN  360.0f
#define REWARD_LOSS -60.0f
#define alpha 0.2f
// Define Object Names
...
// This is the callback function for the object’s contact sensor.
// This function is used to test whether the contact sensor,
// called my_contact, defined for the object in gazebo-arm.world,
// observes a collision with another element/model or not.
//
// Furthermore, this callback function can also be used to define
// a reward function based on whether there has been a collision or not.
	...
	for (unsigned int i = 0; i < contacts->contact_size(); ++i)
	{
		...
		if (collisionCheck)
		{
			...
			if(isArmCollision){
				...
				rewardHistory 	= REWARD_LOSS + 0.5 * avgGoalDelta;
			...
			if(isGripperCollision){
				...
				rewardHistory = REWARD_WIN + 0.8 * (maxEpisodeLength - episodeFrames);
				newReward  = true;
				endEpisode = true;
			}
		}
